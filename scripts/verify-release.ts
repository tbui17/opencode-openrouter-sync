...
async function gh(args: string[]): Promise<string> {
  ...
  if (result.exitCode !== 0) {
    ...
    throw new Error(
      `gh ${args.join(' ')} failed (exit ${result.exitCode}): ${stderr}`,
    );
  }
  return stdout;
}
...
async function getLatestRun(workflow: string): Promise<WorkflowRun | null> {
  try {
    ...
    log(
      `Warning: could not fetch run for ${workflow}: ${(e as Error).message}`,
    );
    return null;
  }
}
...
async function pollWorkflow(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    if (!run) {
      log(
        `  [${attempt}/${maxAttempts}] No runs found yet for ${label}. Waiting...`,
      );
  ...
  fail(
    `${label} did not complete within ${maxAttempts * intervalSec}s. Timed out.`,
  );
}
...
async function verifyVersionsMatch(): Promise<void> {
  ...
  if (!ghTag) {
    log(
      '  No GitHub release found — release-please may not have created one yet. Skipping version match.',
    );
    return;
  }
